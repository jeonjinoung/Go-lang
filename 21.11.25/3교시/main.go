...
포인터는 메모리 주소글 값으로 갖는다.
&를 이용해서 데이터의 메모리 주소를 알수 있다.
포인터를 이용하면 메모리 주소값으로 메모리를 컨트롤할 수 있다.

스텍 메모리가 훨씬효율적이다. ?? 뭐보다?? 
매개변수 지역변수 이러한 녀석들??
함수외부로 공개되는 매모리공간은 hip? 메모리공간 할당??
기본 type은 스텍에 할당한다.

Go 언어는 탈출검사를 통해서 지가 스스로 판단하는데
A라는 변수에 함수 외부로 공개가 되느냐 안되느냐 
스텍이냐 hip이냐?? 할당을 한다 뭐를 통해? 탈출검사를 통해서
동적 메모리 -> 메모리 효율성이 좋다.
재기호출이 이뤄난다. 스텍 오버플로우 현상이 안생긴다. -> 넘치는현상이 안생긴다.


C, c++ 동적메모리할당을 hip에 해주는데
일정인크기를 가자 동적이지 않다 
정적이다.? 스텍의 크기를 따로 정해주지않으면 1mg를 기본적으로 잡아준다.
